  ...
  if [[ -n "$dest" ]]; then
    ...
    while kill -0 "$CURRENT_DOWNLOAD_PID" 2>/dev/null; do
      ...
      if [[ -n "$total_size" && "$total_size" -gt 0 ]]; then
        ...
        local bucket=$(( (pct / 10) * 10 ))

        if (( bucket > last_printed_bucket )); then
          ...
          if [[ -t 1 ]]; then
            ...
            info "Downloading: ${bucket}% (${cur_str}/${total_str})"
          fi
        fi
      else
        # Unknown size: keep the output sparse, but still show activity occasionally.
        if (( elapsed >= 5 )); then
          local cur_str
          cur_str=$(format_size "$current_size")
          local downloaded=$((current_size - start_size))
          ...
          speed_str=$(format_speed "$speed")
          if [[ -t 1 ]]; then
            printf "\r  Downloading: ${cur_str} | ${speed_str} avg\033[K"
          else
    ...
    if [[ -n "$total_size" && "$total_size" -gt 0 && $last_printed_bucket -lt 100 ]]; then
      local current_size
      current_size=$(wc -c < "$dest" 2>/dev/null || echo 0)
      current_size=${current_size//[[:space:]]/}
      local total_str
      ...
      if [[ -t 1 ]]; then
        ...
        if (( cols >= 85 )); then
          local bar_width=20
          local num_fill=bar_width
          local bar=""
          for ((i=0; i<num_fill; i++)); do bar="${bar}${fill}"; done
          for ((i=num_fill; i<bar_width; i++)); do bar="${bar}${empty}"; done
          out_str="  Downloading: [${bar}] 100% (${cur_str}/${total_str}) | ${speed_str} avg"
        elif (( cols >= 70 )); then
          local bar_width=15
          local num_fill=bar_width
          local bar=""
          ...
          for ((i=0; i<num_fill; i++)); do bar="${bar}${fill}"; done
          for ((i=num_fill; i<bar_width; i++)); do bar="${bar}${empty}"; done
          out_str="  Downloading: [${bar}] 100% (${cur_str}/${total_str})"
        elif (( cols >= 40 )); then
          out_str="  Downloading: 100% (${cur_str}/${total_str})"
        else
          out_str="  Downloading: 100%"
        fi
        printf "\r%s\033[K" "$out_str"
      else
        info "Downloading: 100% (${cur_str}/${total_str})"
      fi
    fi
